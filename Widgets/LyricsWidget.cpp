/*$T indentinput.cpp GC 1.140 08/18/17 18:23:17 */
    ...
    QWidget(parent)
{
    ...
    setLine1Y(220);
    setLine2Y(100);
    connect(animation, SIGNAL(valueChanged(QVariant)), this, SLOT(onAnimationValueChanged(QVariant)));
...
{
    if(cursor_index >= cursors.count())return;
    if(tick < cursors[cursor_index])return;
    if(at_end_line)
    {
        at_end_line = false;
        char_index = 0;
    ...
    {
        if(isLine1)setTextLine2(lyrics.at(linesIndex));
        else
    ...
    {
        cursor_toEnd = chars_width.
        count()
        == 0 ? 0 : chars_width.last();
...
void LyricsWidget::setSeekPositionCursor (int tick)
{
    ...
    for(int i = 0; i < round; i++)
    {
        ...
        if(chars_width.count() == 0 && linesIndex < lyrics.count())
        {
            if(isLine1)tLine2 = lyrics.at(linesIndex);
            else
        ...
        if(char_index == chars_width.count())
        {
            ...
            /*
             * if (linesIndex < lyrics.count()) {  if (isLine1)  tLine1 = lyrics.at(linesIndex);
             *   else  tLine2 = lyrics.at(linesIndex);
             *   linesIndex++;
             *   } else {  if (isLine1) tLine1 = "";
             *   else tLine2 = "";
             *   }
             */
            continue;
...
{
    if(isLine1)return getCharsWidth(tLine1);
    else
...
QList<int> LyricsWidget::getCharsWidth (const QString &text)
{
    ...
    for(int i = 0; i < text.length(); i++)
    {
        ...
        width(text.left(i + 1))
        +
        qMax(tBorderWidth, cBorderWidth)
        + 3;
        cl.append(w);
...
QRect LyricsWidget::calculateUpdateArea()
{
    ...
    if(isLine1)
    {
        ...
        r.setWidth(pixLine1.width() + qMax(tBorderWidth, cBorderWidth) + 20);
    ...
    else
    {
        ...
        r.setWidth(pixLine2.width() + qMax(tBorderWidth, cBorderWidth) + 20);
...
{
    QSize s;
    s.setWidth(fontMetrics().width(text) +(qMax(tBorderWidth, cBorderWidth) * 2) + 20);
    s.setHeight(fontMetrics().height());
...
void LyricsWidget::drawTextToPixmap(QPixmap * pix, const QString & text)
{
    ...
    p.setBrush(tColor);
    p.setPen(Qt::NoPen);
    path.addText(qMax(tBorderWidth, cBorderWidth), font().pointSize(), font(), text);
    path.translate(qMax(tBorderWidth, cBorderWidth), 23);
    path.setFillRule(Qt::WindingFill);
    p.drawPath(path);
    if(tBorderWidth > 0)
    {
        QPen pen(Qt::red, tBorderWidth, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin);
        QPainterPathStroker stroker(pen);
        QPainterPath strokedPath = stroker.createStroke(path);
        stroker.setWidth(tBorderOutWidth);
...
void LyricsWidget::drawCursorTextToPixmap(QPixmap * pix, const QString & text)
{
    ...
    if(cBorderWidth > 0)
    {
        QPen pen(Qt::red, cBorderWidth, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin);
        QPainterPathStroker stroker(pen);
        QPainterPath strokedPath = stroker.createStroke(path);
        stroker.setWidth(cBorderOutWidth);
